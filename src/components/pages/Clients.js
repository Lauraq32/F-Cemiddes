...
const Clients = () => {
  ...
  const saveClient = () => {
    ...
    if (client.name.trim()) {
      ...
      if (client._id) {
        axios
          .put(
            `${process.env.REACT_APP_API_URL}/api/patients/` + _client._id,
            _client,
            { headers: getHeaders() }
          )
          .then((response) => {
        ...
        axios
          .post(`${process.env.REACT_APP_API_URL}/api/patients`, _client, {
            headers: getHeaders(),
          })
          .then((response) => {
            toast.current.show({
  ...
  const deleteClient = () => {
    ...
    setClient(emptyClient);
    axios
      .delete(`${process.env.REACT_APP_API_URL}/api/patients/` + client._id, {
        headers: getHeaders(),
      })
      .then((response) => {
        toast.current.show({
  ...
  const nameBodyTemplate = (rowData) => {
    return <NavLink to={`/clients/${rowData._id}`}>{rowData.name}</NavLink>;
  };
  ...
  const statusOptions = [
    ...
    {id: 2, name: 'Inactivo', status: 'inactivo'}
  ]
  ...
  const updateStatus = (e) => {
    setStatus(e.value)
    setClient({...client, status: e.value.status})
  }
    ...
    return (
      <div className="datatable-actions">
        <Button
          ...
          onClick={() => showSelectedClientDialog(rowData)}
        />
        <Button
          icon="pi pi-pencil"
        ...
        <div className="card">
          ...
          >
            <div className="field">
              ...
              <InputText
                ...
                className={classNames({
                  "p-invalid": submitted && !client.name,
                })}
              />
            ...
            <div className="field">
              ...
              <InputText
                ...
                required
                className={classNames({
                  "p-invalid": submitted && !client.phone,
                })}
              />
            </div>
            ...
            <div className="field">
              ...
              <InputText
                ...
                required
                className={classNames({
                  "p-invalid": submitted && !client.email,
                })}
              />
              {submitted && !client.email && (
                <small className="p-invalid">
                  se necesita agregar el email
                </small>
              )}
            </div>
          ...
          >
            <div className="flex align-items-center justify-content-center">
              ...
              {client && (
                <span>
                  ¿Estás seguro de que quieres eliminar <b>{client.patient}</b>?
                </span>
              )}
            </div>
          </Dialog>
          ...
          {dialogIsVisible && (
            ...
            >
              <div className="modal-content">
                <p>
                  <b>Paciente:</b> {dialogContent.name}
                </p>
                <p>
                  <b>Tel&eacute;fono:</b> {dialogContent.phone}
                </p>
                <p>
                  <b>Correo electr&oacute;nico:</b> {dialogContent.email}
                </p>
                <p>
                  <b>Visitas:</b> {dialogContent.visitas}
                </p>
              </div>
            </Dialog>
          )}
