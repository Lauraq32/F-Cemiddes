...
const Products = () => {
  ...
  const getAllCuotas = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/patients/treatments`, {
        headers: getHeaders(),
      })
      .then((response) => {
  ...
  const getAllTreatments = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/treatments`, {
        headers: getHeaders(),
      })
      .then((response) => {
  ...
  const saveProduct = () => {
    ...
    if (cuota._id) {
      axios
        .put(
          `${process.env.REACT_APP_API_URL}/api/patients/treatments/` +
            _cuota._id,
          _cuota,
          { headers: getHeaders() }
        )
  ...
  const deleteProduct = () => {
    ...
    axios
      .delete(
        `${process.env.REACT_APP_API_URL}/api/patients/treatments/` + cuota._id,
        { headers: getHeaders() }
      )
      .then((response) => {
        toast.current.show({
  ...
  const statusOptions = [
    { id: 1, name: "Activo", status: "activo" },
    { id: 2, name: "Suspendido", status: "suspendido" },
    { id: 3, name: "completado", status: "completado" },
  ];
    ...
    return (
      <div className="datatable-actions">
        <Button
          ...
          onClick={() => showDueDetailsDialog(rowData)}
        />
        <Button
          icon="pi pi-pencil"
      ...
      <div className="col-12">
        <div className="card">
          ...
          <Dialog
            ...
            style={{ width: "450px" }}
            header="Detalles de las cuotas"
            modal
            className="p-fluid"
            footer={productDialogFooter}
            onHide={hideDuesDialog}
          >
            <div className="field">
              ...
              <Dropdown
                ...
                onChange={(e) => setDropDownTreatment(e.value)}
                options={dropDownTreatmentValues}
                optionLabel="treatment"
                placeholder="Seleccionar"
              />
              {submitted && !cuota.treatmentId && (
            ...
            <div className="field col">
              ...
              <Dropdown
                ...
                onChange={(e) => setDropDownClient(e.value)}
          ...
          >
            <div className="flex align-items-center justify-content-center">
              ...
              {cuota && (
                <span>
                  ¿Estás seguro de que quieres eliminar <b>{cuota.cuotas}</b>?
                </span>
              )}
            </div>
          </Dialog>
          <Dialog
            visible={deleteCuotasDialog}
            style={{ width: "450px" }}
            header="Confirmar"
            modal
          ...
          >
            <div className="flex align-items-center justify-content-center">
              ...
              {cuota && (
                <span>
                  ¿Está seguro de que desea eliminar los productos
                  seleccionados?
                </span>
              )}
            </div>
          ...
          {dialogIsVisible && (
            ...
            >
              <div className="modal-content">
                <p>
                  <b>Tratamiento:</b> {dialogContent.treatment.name}
                </p>
                <p>
                  <b>Nombre:</b> {dialogContent.patient.name}
                </p>
                <p>
                  <b>Total del tratamiento:</b>{" "}
                  {formatCurrency(
                    dialogContent.deuda + dialogContent.totalAmountPaid
                  )}
                </p>
                <p>
                  <b>Monto pagado:</b>{" "}
                  {formatCurrency(dialogContent.totalAmountPaid)}
                </p>
                <p>
                  <b>Deuda:</b> {formatCurrency(dialogContent.deuda)}
                </p>
              </div>
            </Dialog>
          )}
